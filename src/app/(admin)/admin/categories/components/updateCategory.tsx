...
import { Category, UpdateCategory as UpdateCategoryType } from '@/types/category.type'
...
export default function UpdateCategory({ category }: UpdateCategoryProps) {
  const form = useForm<z.infer<typeof categorySchema.updateCategory>>({
    resolver: zodResolver(categorySchema.updateCategory),
    defaultValues: {
      category_name: '',
      category_image: {},
  ...
  const categoryImageUrl = form.watch('category_image.url')

  const [avatarPreview, setAvatarPreview] = useState<avatarPreviewType | null>(null)
  const { mutate: updateCategory, isPending: isPendingUpdateCategory } = useCategory.updateCategory(category._id)
  ...
  const removeFile = () => {
    ...
    form.setValue('category_image.url', category.category_image.url || '')
  }
  ...
  const handleSubmit = async (data: z.infer<typeof categorySchema.updateCategory>) => {
    let payload: Partial<UpdateCategoryType> = {
  ...
  useEffect(() => {
    if (category) {
      ...
      setAvatarPreview({
        ...
        name: category.category_image.url,
        size: 0,
      })
    }
  }, [])
        ...
        >
          <div className="flex justify-between items-center gap-8 w-full flex-col p-2">
            <div className="w-full flex justify-end items-center">
              <Button
                disabled={isPendingUpdateCategory || isPendingUploadImage}
                type="submit"
                className=" h-12  bg-blue-tertiary hover:bg-blue-tertiary/90 rounded-2xl"
              >
                Update category
                {(isPendingUpdateCategory || isPendingUploadImage) && <Loading />}
              </Button>
            </div>
                ...
                render={({ field }) => (
                  <FormItem className="w-full">
                    <div className="relative w-full">
                      ...
                      <FloatingLabel htmlFor="category_description">Description</FloatingLabel>
                    </div>
                    {form.formState.errors.category_description && (
                      <p className="text-red-500 text-sm mt-2">{form.formState.errors.category_description.message}</p>
                    )}
                  </FormItem>
                )}
              />
            ...
            <div className="w-full grid grid-cols-2 gap-8">
              <div className="relative w-full overflow-hidden h-[400px] border border-blue-primary rounded-[12px]">
                <Image
                  src={avatarPreview?.src || categoryImageUrl || '/images/default_product_image.png'}
                  alt="product image"
                  fill
                  objectFit="cover"
                />
